...
def test_table_services_requests_ok(client, db_session):
    ...
    assert data[0]["id"] == srv.id
    assert data[0]["reservation"]["id"] == res.id
    assert data[0]["reservation"]["room"]["room_number"] == room.room_number
    assert data[0]["reservation"]["guest"]["name"] == guest.name
    assert data[0]["status"] == srv.status
    assert data[0]['created_at'] == srv.created_at.isoformat()